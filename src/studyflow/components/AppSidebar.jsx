...
import { Box, Stack, Typography, Button, IconButton, LinearProgress } from '@mui/material';
    ...
    <Fragment>
        ...
        <Box component="aside"
            sx={{
                ...
                borderRight: `1px solid ${COLORS.white05}`, transition: 'all 0.3s', zIndex: 200, flexShrink: 0,
                backdropFilter: 'blur(48px)', background: COLORS.overlayDark,
            ...
            <Box component="nav" sx={{ flex: 1, overflowY: 'auto', scrollbarWidth: 'none', '&::-webkit-scrollbar': { display: 'none' }, py: 1.5, px: 1, display: 'flex', flexDirection: 'column', gap: 0.5 }}>
                ...
                <Box sx={{ my: 1, borderTop: `1px solid ${COLORS.white05}`, mx: 1 }} />
                ...
                {sidebarOpen && (
                    <Box sx={{ mt: 2, px: 0.5 }}>
                        <Box sx={{ height: '1px', width: '100%', mb: 2, mx: 0.5, background: `linear-gradient(90deg, transparent, ${COLORS.accent}40, transparent)` }} />
                        <Stack direction="row" justifyContent="space-between" alignItems="center" mb={1.5} px={1}>
                            <Typography sx={{ fontSize: 12, fontFamily: '"Clash Display", sans-serif', fontWeight: 700, color: COLORS.textDim, textTransform: 'uppercase', letterSpacing: '0.05em' }}>Moje kurzy</Typography>
                            <IconButton size="small" onClick={() => setActiveNav('Poznámky')}
                                sx={{ width: 24, height: 24, borderRadius: 2, color: COLORS.white30, '&:hover': { color: COLORS.accent, bgcolor: `${COLORS.accent}1a`, boxShadow: `0 0 10px ${COLORS.accent}4d` } }}>
                                <Plus size={14} />
                            </IconButton>
                        </Stack>
                        <Stack gap={0.5}>
                        ...
                        <Box sx={{ my: 3, borderTop: `1px solid ${COLORS.white05}`, mx: 1 }} />
                        
                        <Stack gap={1} px={1} mb={2}>
                            <Typography sx={{ fontSize: 11, fontWeight: 700, color: COLORS.textDim, textTransform: 'uppercase', letterSpacing: '0.05em', mb: 0.5 }}>Správa dat</Typography>
                            <Button 
                                size="small" 
                                startIcon={<Download size={14} />} 
                                onClick={exportData}
                                sx={{ 
                                    justifyContent: 'flex-start', color: COLORS.white50, textTransform: 'none', fontSize: 12, 
                                    '&:hover': { color: COLORS.primary, background: `${COLORS.primary}0D` } 
                                }}
                            >
                                Exportovat data
                            </Button>
                            <Button 
                                ...
                                sx={{ 
                                    justifyContent: 'flex-start', color: COLORS.white50, textTransform: 'none', fontSize: 12,
                                    '&:hover': { color: COLORS.green, background: `${COLORS.green}0D` } 
                                }}
                            >
                                Importovat data
                                <input
