...
export const TopicCard = ({ topic, courseColor, stats = {}, onOpen }) => {
    ...
    const move = (e) => {
        ...
        setHStyle({
            background: `radial-gradient(circle at ${x * 100}% ${y * 100}%, ${COLORS.white04}, transparent 60%)`,
...
export const AddTopicModal = ({ courseColor, onSave, onClose }) => {
    ...
    return (
        <Dialog open onClose={onClose} maxWidth="xs" fullWidth
            slotProps={{ paper: { sx: { ...DIALOG_PAPER_SX, animation: `${modalSlideInAnim} 0.22s ease-out both` } } }}>
            <DialogTitle sx={{ pb: 0 }}>
                <Stack direction="row" justifyContent="space-between" alignItems="center">
                    <Typography variant="h6" fontWeight={700} fontFamily="'Clash Display', sans-serif">Nové téma</Typography>
                    <IconButton onClick={onClose} size="small" sx={{ color: COLORS.white30 }}><X size={16} /></IconButton>
                </Stack>
            </DialogTitle>
            <DialogContent>
                <Stack gap={2} sx={{ pt: 1 }}>
        ...
        <Box sx={{ maxWidth: 1140, mx: 'auto' }}>
            ...
            <Stack direction="row" alignItems="center" gap={1.5} mb={3}>
                <Button onClick={onBack} startIcon={<ChevronLeft size={16} />} variant="outlined"
                    sx={{ borderRadius: 3, textTransform: 'none', fontSize: 13, fontWeight: 500, color: COLORS.white45, borderColor: COLORS.white08, '&:hover': { bgcolor: COLORS.white05 } }}>
                    Zpět
                </Button>
                <Typography sx={{ color: COLORS.white20 }}>/</Typography>
                <Typography sx={{ fontSize: 13, fontWeight: 500, color: COLORS.white50 }}>Poznámky</Typography>
                <Typography sx={{ color: COLORS.white20 }}>/</Typography>
                <Typography sx={{ fontSize: 13, fontWeight: 600, color: 'white' }}>{course.name}</Typography>
            </Stack>

            {/* Course hero */}
            <Paper elevation={0} sx={{ position: 'relative', borderRadius: 4, p: 3, mb: 3, overflow: 'hidden', background: COLORS.white02, border: `1px solid ${COLORS.white07}` }}>
                ...
                <Stack direction="row" alignItems="flex-start" gap={2.5} flexWrap="wrap">
                    ...
                    <Box sx={{ flex: 1, minWidth: 0 }}>
                        ...
                        {course.description && <Typography sx={{ fontSize: 14, color: COLORS.white45, lineHeight: 1.5 }}>{course.description}</Typography>}
                    </Box>
            ...
            <Stack direction="row" alignItems="center" justifyContent="space-between" mb={2}>
                <Stack direction="row" alignItems="center" gap={1}>
                    ...
                    <Box component="span" sx={{ display: 'inline-flex', alignItems: 'center', justifyContent: 'center', height: 22, px: 1, borderRadius: 99, fontSize: 11, fontFamily: 'monospace', bgcolor: COLORS.white05, color: COLORS.white40 }}>{topics.length}</Box>
                </Stack>
            ...
            {topics.length === 0 ? (
                <Stack alignItems="center" justifyContent="center" sx={{ py: 10, textAlign: 'center', border: `1px dashed ${COLORS.white08}`, borderRadius: 4 }}>
                    ...
                    <Typography sx={{ fontSize: 13, color: COLORS.white35, maxWidth: 260, mb: 2.5 }}>Přidejte první téma a začněte organizovat poznámky.</Typography>
                ...
                <Box sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr', sm: '1fr 1fr', lg: 'repeat(3, 1fr)' }, gap: 1.5 }}>
                    ...
                    <Button onClick={() => setShowAddTopic(true)} variant="outlined"
                        sx={{ borderRadius: 3, p: 2, borderStyle: 'dashed', borderColor: COLORS.white09, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 1, minHeight: 80, textTransform: 'none', fontSize: 13, fontWeight: 500, color: COLORS.white25, '&:hover': { bgcolor: COLORS.white03, borderColor: COLORS.white20 } }}>
        ...
        <Box sx={{ maxWidth: 1140, mx: 'auto' }}>
            ...
            <Stack direction="row" alignItems="flex-start" justifyContent="space-between" mb={3.5} gap={2}>
                <Box>
                    ...
                    <Typography sx={{ fontSize: 13, color: COLORS.white35 }}>
                        {courses.length} {coursesLabel} · {totalNotes} poznámek celkem
                    </Typography>
                </Box>
                <Button onClick={() => setShowAddModal(true)} startIcon={<Plus size={16} />}
                    sx={{ borderRadius: 3, textTransform: 'none', fontSize: 13, fontWeight: 600, flexShrink: 0, px: 2, py: 1.25, background: COLORS.blue12, color: COLORS.blue, border: `1px solid ${COLORS.blue22}`, '&:hover': { background: COLORS.blue20, transform: 'scale(1.02)' }, '&:active': { transform: 'scale(0.98)' } }}>
                    Nový kurz
                </Button>
            </Stack>
            ...
            {courses.length === 0 ? (
                <Stack alignItems="center" justifyContent="center" sx={{ py: 14, textAlign: 'center', border: '1px dashed rgba(255,255,255,0.08)', borderRadius: 4 }}>
                    <Box sx={{ width: 64, height: 64, borderRadius: 4, display: 'flex', alignItems: 'center', justifyContent: 'center', mb: 2, background: COLORS.blue08 }}>
                        <Folder size={28} style={{ color: COLORS.blue }} />
                    </Box>
                    ...
                    <Typography sx={{ fontSize: 13, color: COLORS.white35, maxWidth: 280, mb: 3 }}>Vytvořte první kurz a začněte organizovat poznámky a kartičky.</Typography>
                ...
                <Box sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr', sm: '1fr 1fr', lg: 'repeat(3, 1fr)', xl: 'repeat(4, 1fr)' }, gap: 2 }}>
                    ...
                    <Button onClick={() => setShowAddModal(true)} variant="outlined"
                        sx={{ borderRadius: 4, p: 2.5, borderStyle: 'dashed', borderColor: COLORS.white09, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 1.5, minHeight: 220, textTransform: 'none', '&:hover': { bgcolor: COLORS.white03, borderColor: COLORS.white20 } }}>
                        <Box sx={{ width: 40, height: 40, borderRadius: 3, display: 'flex', alignItems: 'center', justifyContent: 'center', background: COLORS.white04 }}>
                            <Plus size={20} style={{ color: COLORS.white30 }} />
                        </Box>
                        <Typography sx={{ fontSize: 13, fontWeight: 500, color: COLORS.white25 }}>Přidat kurz</Typography>
                    </Button>
                </Box>
            )}
